...
```
### 摘要
    增强Java语言以使用初始化值将类型推断扩展为局部变量的声明。
### 目标
...
### 成功的标准
    定量上，我们希望实际代码库中的大部分局部变量声明都能够使用此特性转换，进而推断出合适的类型。
...
```
    ...
    缺少初始化值的局部变量声明的形式、声明多个变量，有额外的数组维度符号[]、或引用正在初始化的变量，这些都是不被允许的。
...
*   允许捕获类型的变量进入后续的语句会增加语言的表现力，但这不是此特性的目标。相反，这种建议的映射操作是我们无论如何都
